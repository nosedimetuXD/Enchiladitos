...
function UserStatsProfileModal({ selectedUser, sales, onClose, loading }) {
...
export default function Users() {
  ...
  async function openStatsModal(userItem) {
    ...
    try {
      const [sData, eData, tData, wData] = await Promise.all([
        api.get('/sales?period=all').catch(() => []),
        api.get('/expenses?period=all').catch(() => []),
        api.get('/tasks').catch(() => []),
        api.get('/waste').catch(() => [])
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs font-bold text-amber-600 dark:text-amber-400 mt-0.5">
            Cuentas, credenciales, rendimiento individual y permisos del equipo Enchiladitos
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <select
              ...
              className="w-full px-3.5 py-2.5 rounded-2xl bg-white dark:bg-[#150904] border border-[#D4B28E] dark:border-[#9F6839]/60 text-sm font-semibold text-[#432414] dark:text-[#FEE4D7]"
            >
              <option value="employee">Empleado (Ventas, Inventario lectura)</option>
