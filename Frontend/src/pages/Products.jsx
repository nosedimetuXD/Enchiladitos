...
import { Flame, Plus, Edit2, Trash2, Search, BookOpen, Image as ImageIcon, Upload, CheckCircle2, XCircle } from 'lucide-react'
...
export default function Products() {
  ...
  async function toggleProductActive(prod, e) {
    ...
    try {
      await api.put(`/products/${prod.id}`, {
        ...
        price: prod.price,
        category: prod.category || 'Gomitas',
  ...
  if (loading) return <p className="p-4 text-sm font-semibold text-red-600">Cargando productos...</p>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-black text-[#450a0a] dark:text-[#fef2f2] tracking-tight">
            Catálogo de Productos & Menú Enchiladitos
          </h2>
          <p className="text-xs font-bold text-amber-600 dark:text-amber-400 mt-0.5">
            Precios, recetas de insumos consumidos y disponibilidad en caja
          </p>
        </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full bg-white dark:bg-[#1c0707] border border-red-200 dark:border-red-950/60 focus:border-red-500 rounded-2xl pl-10 pr-3 py-2.5 text-xs font-bold text-[#450a0a] dark:text-[#fef2f2] focus:outline-none shadow-xs"
          />
        </div>
            ...
            <button
              ...
              className={`px-3.5 py-2 rounded-2xl text-xs font-black whitespace-nowrap transition-all cursor-pointer ${
                selectedCategory === cat
                  ...
                  : 'bg-white dark:bg-[#1c0707] border border-red-200 dark:border-red-950/60 text-[#450a0a] dark:text-[#fef2f2]'
              }`}
            >
              {cat}
            </button>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {filtered.map((prod) => {
          ...
          return (
            <div
              key={prod.id}
              className={`bg-white dark:bg-[#1c0707] border border-red-200 dark:border-red-950/60 rounded-3xl overflow-hidden flex flex-col justify-between shadow-xs transition-all ${
                !activeState ? 'opacity-65' : ''
              }`}
            >
              <div>
                <div className="relative h-36 w-full bg-red-50 dark:bg-[#140505] overflow-hidden">
                  <img
                    src={img}
                    alt={prod.name}
                  ...
                  <div className="absolute top-2.5 left-2.5">
                    <span className="px-2.5 py-0.5 rounded-full bg-[#140505]/90 text-amber-300 font-black text-[10px] backdrop-blur-xs shadow-xs border border-red-900">
                ...
                <div className="p-4">
                  <h3 className="font-black text-base text-[#450a0a] dark:text-[#fef2f2]">{prod.name}</h3>
                  {prod.description && (
                    <p className="text-xs text-amber-700 dark:text-amber-400 line-clamp-2 mt-1 font-semibold">
                      {prod.description}
                    </p>
                  )}
                </div>
              ...
              <div className="px-4 pb-4 pt-2 border-t border-red-200/60 dark:border-red-950 flex items-center justify-between">
                <div>
                  <span className="text-[10px] text-amber-600 font-bold block">Precio Venta</span>
                  <span className="text-lg font-black text-[#450a0a] dark:text-[#fef2f2]">
                ...
                <div className="flex items-center gap-1.5">
                  <Link
                    ...
                    className="p-2 rounded-xl bg-red-50 dark:bg-red-950/60 text-red-600 dark:text-amber-400 hover:bg-red-600 hover:text-white transition-colors flex items-center gap-1 text-xs font-black"
                    title={isEmployee ? "Ver Receta (Modo Lectura)" : "Ver / Editar Receta de Insumos"}
                  >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder="Ej. Gomitas Escarchadas 10 unidades"
              required
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input
                ...
                placeholder="Ej. Gomitas, Sparkies"
                required
                className="w-full px-3.5 py-2.5 rounded-2xl bg-white dark:bg-[#140505] border border-red-200 dark:border-red-950 text-sm font-bold text-[#450a0a] dark:text-[#fef2f2] focus:outline-none focus:ring-2 focus:ring-red-500"
              />
              <datalist id="category-suggestions">
                <option value="Gomitas" />
                <option value="Sparkies" />
                <option value="Escarchados" />
          ...
          <div className="space-y-2">
            <label className="block text-xs font-black text-[#450a0a] dark:text-amber-300 uppercase tracking-wider mb-1 flex items-center gap-1.5">
              <ImageIcon className="w-3.5 h-3.5 text-red-500" /> Imagen del Producto
            </label>

            <div className="flex items-center gap-3">
              <label className="inline-flex items-center gap-2 px-3.5 py-2 rounded-2xl bg-red-50 dark:bg-[#240a0a] border border-red-200 dark:border-red-950 hover:bg-red-600 hover:text-white text-xs font-black text-[#450a0a] dark:text-[#fef2f2] transition-all cursor-pointer shadow-xs">
              ...
              <span className="text-[11px] text-amber-700 font-semibold">o escribe una URL</span>
            </div>
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Notas de sabor, tajín, chamoy o porción..."
              className="w-full px-3.5 py-2.5 rounded-2xl bg-white dark:bg-[#140505] border border-red-200 dark:border-red-950 text-sm font-semibold text-[#450a0a] dark:text-[#fef2f2] focus:outline-none focus:ring-2 focus:ring-red-500"
            />
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            />
            <label htmlFor="is-active-check" className="text-xs font-bold text-[#450a0a] dark:text-[#fef2f2] cursor-pointer">
              Producto Activo y visible para venta en POS
            </label>
          </div>
