  ...
  return (
    <div className="relative w-full select-none" style={{ height: `${height}px` }}>
      ...
      >
          ...
          return (
            <g key={`grid-h-${idx}`}>
              <line
                ...
                y2={y}
                stroke="currentColor"
                className="text-red-200/40 dark:text-red-950/50"
                strokeWidth="1"
              ...
              <text
                ...
                y={y + 3}
                textAnchor="end"
                className="fill-red-900/80 dark:fill-red-300/70 text-[9px] font-mono tabular-nums"
              >
                {labelVal >= 1000000
        ...
        {points1.map((p, idx) => (
          <line
            ...
            stroke="currentColor"
            className="text-red-200/25 dark:text-red-950/30"
            strokeWidth="1"
            strokeDasharray="2 2"
          />
        ))}
          ...
          points2.map((p, idx) => {
            ...
            return (
              <circle
                ...
                r={isHovered ? 5.5 : 3.5}
                fill={line2Color}
                className="stroke-white dark:stroke-[#1c0707] transition-all duration-150"
                strokeWidth={isHovered ? 2.5 : 1.5}
              />
            )
          })}

        {/* Puntos Serie 1 (Círculos) */}
        {points1.map((p, idx) => {
          const isHovered = hoverIndex === idx
          return (
            <circle
          ...
          return (
            <text
              ...
              className={
                'text-[9px] font-mono ' +
                (hoverIndex === idx
                  ? 'fill-[#450a0a] dark:fill-[#fef2f2] font-bold'
                  : 'fill-red-900/70 dark:fill-red-400/60 font-normal')
              }
            >
              {p.data.label}
            </text>
      ...
      {hoverIndex !== null && activePoint1 && (
        <div
          className="absolute pointer-events-none z-30 transition-all duration-100 ease-out bg-[#200808] dark:bg-[#140505] text-[#fff5f2] border border-red-900/40 rounded-xl px-2.5 py-1.5 shadow-xl text-left"
          style={{
        ...
        >
          <div className="text-[10px] font-semibold text-amber-400 border-b border-red-900/30 pb-0.5 mb-1 whitespace-nowrap">
          ...
          {hasSecondary && activePoint2 && (
            <div className="flex items-center gap-1.5 text-xs font-bold text-amber-300 mt-0.5 whitespace-nowrap">
