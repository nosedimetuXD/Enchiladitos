    ...
    <div className="flex h-screen w-screen overflow-hidden bg-[#fff5f2] dark:bg-[#140505] text-[#450a0a] dark:text-[#fef2f2] transition-colors duration-200">
      ...
      >
        ...
        <div className="px-3 py-2 border-t border-red-200/60 dark:border-red-950/60 bg-red-50/40 dark:bg-[#140505]">
          {!isCollapsed ? (
            <div className="flex items-center justify-between p-2 rounded-xl bg-white dark:bg-[#200808] border border-red-200/60 dark:border-red-950/40 shadow-xs">
              <div className="flex items-center gap-2">
                <div
                  className={`p-1.5 rounded-lg transition-colors ${
                    isDarkMode ? 'bg-red-950/80 text-amber-400' : 'bg-red-100/70 text-red-600'
                  }`}
                >
                  {isDarkMode ? <Moon className="w-3.5 h-3.5 text-amber-400" /> : <Sun className="w-3.5 h-3.5 text-red-600" />}
                </div>
                <div className="flex flex-col">
                  <span className="text-xs font-bold text-[#450a0a] dark:text-[#fef2f2] leading-none">
                    {isDarkMode ? 'Modo Oscuro' : 'Modo Claro'}
                  </span>
                  <span className="text-[9px] font-semibold text-amber-600 dark:text-amber-400 mt-0.5 leading-none">
                    Paleta Enchiladitos
                  </span>
                </div>
              </div>
              ...
              <button
                ...
                className={`relative inline-flex h-5 w-10 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out ${
                  isDarkMode ? 'bg-red-600' : 'bg-red-200'
                }`}
                title={isDarkMode ? 'Cambiar a Modo Claro' : 'Cambiar a Modo Oscuro'}
              >
                <span
                  className={`pointer-events-none inline-flex items-center justify-center h-4 w-4 transform rounded-full bg-white dark:bg-[#fef2f2] shadow-xs transition duration-200 ${
                    isDarkMode ? 'translate-x-5' : 'translate-x-0'
                  }`}
                >
                  {isDarkMode ? <Moon className="w-2 h-2 text-red-600" /> : <Sun className="w-2 h-2 text-amber-600" />}
                </span>
              </button>
            </div>
          ) : (
            <div className="flex justify-center">
              <button
                ...
                className={`p-2 rounded-xl border transition-all cursor-pointer ${
                  isDarkMode
                    ? 'bg-[#200808] border-red-950/60 text-amber-400 hover:bg-red-950/60'
                    : 'bg-white border-red-200/60 text-red-600 hover:bg-red-50'
                }`}
                title={isDarkMode ? 'Cambiar a Modo Claro' : 'Cambiar a Modo Oscuro'}
              >
                {isDarkMode ? <Moon className="w-3.5 h-3.5 text-amber-400" /> : <Sun className="w-3.5 h-3.5 text-red-600" />}
        ...
        {/* Active User Footer */}
        <div className="p-3 border-t border-red-200/40 dark:border-red-950/40 bg-red-50/40 dark:bg-[#1a0606]">
          <div className={`flex items-center ${isCollapsed ? 'justify-center' : 'justify-between gap-2'}`}>
            <div
              onClick={() => navigate('/profile')}
              className={`flex items-center gap-2 cursor-pointer group ${isCollapsed ? 'justify-center' : 'overflow-hidden text-left flex-1 min-w-0'}`}
              title="Ver mi perfil"
            >
              {userAvatarUrl ? (
                <img
                  src={userAvatarUrl}
                  alt={user?.username}
                  className="w-8 h-8 rounded-full object-cover border border-red-500/50 shrink-0 shadow-xs group-hover:scale-105 transition-transform"
                />
              ) : (
                <div className="w-8 h-8 rounded-full bg-gradient-to-br from-red-600 to-amber-600 text-white font-bold flex items-center justify-center text-xs shrink-0 shadow-xs group-hover:scale-105 transition-transform">
              ...
              {!isCollapsed && (
                <div className="flex flex-col min-w-0">
                  <span className="text-xs font-semibold text-[#450a0a] dark:text-[#fef2f2] truncate group-hover:text-red-600 transition-colors">
                    {user?.username || 'Usuario'}
                  </span>
                  <span className="inline-block mt-0.5">
                    <span className="text-[9px] font-semibold px-1.5 py-0.2 rounded-full bg-white dark:bg-[#140505] text-red-700 dark:text-amber-400 border border-red-200/50 dark:border-red-950/40 uppercase tracking-wider">
                      {user?.role === 'owner' ? 'Dueño' : user?.role === 'admin' ? 'Administrador' : user?.role === 'employee' ? 'Empleado' : (user?.role || 'Personal')}
                    </span>
                  </span>
                </div>
              )}
            ...
            {!isCollapsed && (
              <button
                ...
                className="p-1.5 rounded-lg text-red-600 dark:text-amber-400 hover:text-red-700 hover:bg-red-100 dark:hover:bg-red-950/40 transition-colors shrink-0 cursor-pointer"
                title="Cerrar sesión"
              >
